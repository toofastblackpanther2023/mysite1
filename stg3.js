...
function pageChange(event) {
  if (event.keyCode === 32) {
    window.location.href = "stg4.html"; // [スペースキー]が押されたら新しいページに遷移
  }
}
...
function mainloop() {
  ...
  switch (idx) {
    ...
    case 1: //ゲーム中
      ...
      if (tmr < 30 * 4) fText("STAGE3  "  , 600, 300, 50, "cyan");
      // ステージクリア時に "STAGE CLEAR" というメッセージが表示
      if (30 * 57 < tmr && tmr < 30 * 59)
  ...
  fText("『保全計画』を作成する上で『他部署の要望』を確認することも大事である。", 600, 150, 30, "white");
  fText("『場所』と『部署』の組み合わせで『正しい』のはどれですか？　　　　　　", 600, 200, 30, "white");
}
